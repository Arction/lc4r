      ...
      renderValue: function(args) {
        ...
        components.forEach(component => {
          ...
          ifTry(id_ === 'series', _ => {
            ...
            const pointShape = point_shape && typeof point_shape === 'string' ? {'square' : PointShape.Square, 'circle': PointShape.Circle, 'triangle':  PointShape.Triangle}[point_shape.toLowerCase()] : PointShape.Circle
            ...
            ifTry('setPointSize' in series && typeof point_size === 'number', _ => {
              series.setPointSize(point_size)
            })
            ifTry('setIndividualPointSizeEnabled' in series && typeof point_size === 'object', _ => {
              pointSizes = Array.from(point_size)
              series.setIndividualPointSizeEnabled(true)
            })
            ifTry('setPointFillStyle' in series && point_color, _ => {
              series.setPointFillStyle(new SolidFill({color: parseColor(lcjs, point_color)}))
            })
            ifTry('setStrokeStyle' in series && typeof line_thickness === 'number', _ => {
              series.setStrokeStyle(stroke => stroke.setThickness(line_thickness))
            })
            ifTry('setStrokeStyle' in series && line_color, _ => {
              series.setStrokeStyle(stroke => stroke.setFillStyle(new SolidFill({color: parseColor(lcjs, line_color)})))
            })

            // #region Construct LCJS data points array and push to series
            const pointsCount = Math.max(coordsX ? coordsX.length : 0, coordsY ? coordsY.length : 0)
